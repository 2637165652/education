<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>已撤销家教</el-breadcrumb-item>
    </el-breadcrumb>
    <el-table :data="dataList" stripe style="width: 100%" border v-loading="isLoading">
      ...
      <el-table-column prop="cancelDate" label="撤销日期" >
        <template slot-scope="{ row }">
          {{ row.cancelDate | moment }}
        </template>
      </el-table-column>
      <el-table-column prop="" label="操作" >
        <template slot-scope="{ row }">
          <el-button type="text" @click="handleRepublish(row)">重新发布</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 发布家教 -->
...
<script>
import { getCanceledList, republish } from '@/api/record/index'
export default {
  name: 'Canceled',
  data () {
    return {
  ...
  methods: {
    getDataList () {
      ...
      getCanceledList(userId).then(res => {
